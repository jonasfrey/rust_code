...
fn main() -> std::io::Result<()> {
    ...
    // let s_filename = "bitmap_image_"..".bmp";
    {
        ...
        let n_header_size = (n_index_start - n_bitmap_fileheader_size); 
        let a_n_header_size = f_a_convert_u32_to_4_u8_values(n_header_size);

        println!("n_header_size: {}", n_header_size);
        println!("a_n_header_size: {:?}", a_n_header_size);
        a_array[14] = a_n_header_size[3];
        a_array[15] = a_n_header_size[2];
        a_array[16] = a_n_header_size[1];
        a_array[17] = a_n_header_size[0];
