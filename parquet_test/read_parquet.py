...
class O_config: 
    def __init__(self):
        self.s_path_file = "./o_config.json"
        self.o_dict = {
            's_url': ""
        }

    def f_write(self):
        o_f = open(self.s_path_file, "w")
        o_f.write(json.dumps(self.o_dict))
        o_f.close()
        print(f"{self.s_path_file}: file has been written!")

    def f_read(self):
        o_f = open(self.s_path_file)
        o_dict = json.load(o_f)
        self.o_dict = o_dict
...
# Opening JSON file
o_file_0 = open('./irsa_ipac_data/a_s_url_https_irsa_ipac_caltech_edu_data_ZTF_lc_lc_dr11_0.json')
o_file_1 = open('./irsa_ipac_data/a_s_url__https__irsa_ipac_caltech_edu_data_ZTF_lc_lc_dr11_1.json')
...
if( o_config.o_dict["s_url"] in a_s_url__merged):
    n_index_start_url = a_s_url__merged.index(o_config.o_dict["s_url"])
else: 
    n_index_start_url = 0
...
print("n_index_start_url")
print(n_index_start_url)
n_len_a_s_url__merged = len(a_s_url__merged)
for s_url in a_s_url__merged[n_index_start_url:]:
    ...
    # delete the file
    os.remove(s_path_file)

    # write the current url to config
    o_config.o_dict["s_url"] = s_url
    o_config.f_write()
...
#             a_a_value__filtered,
#             dtype=[('objectid', numpy.dtype('int64')),
#              ('filterid', numpy.dtype('uint8')),
#              ('fieldid', numpy.dtype('int16')),
#              ('rcid', numpy.dtype('uint8')),
#              ('objra', numpy.dtype('float32')),
#              ('objdec', numpy.dtype('float32')),
#              ('nepochs', numpy.dtype('int64')),
#              ('hmjd', numpy.dtype('O')),
#              ('mag', numpy.dtype('O')),
#              ('magerr', numpy.dtype('O')),
#              ('clrcoeff', numpy.dtype('O')),
#              ('catflags', numpy.dtype('O'))] 
#             # dtype=a_a_type
#             # [
#             #     (s,s)
#             #     for 
